    ...
    if (e.target.classList.contains("edit-btn")) {
        const id = e.target.getAttribute("data-id");
        const employee = EmployeeService.getEmployees().find(
        (emp) => emp.id === id
        );
    ...
    if (departmentQuery) {
        ...
        (emp) => emp.department === departmentQuery
        );
    }

    if (maxSalary)
        //Made it so the default of all salaries is 0 meaning no filter is applied
        filteredEmployees = filteredEmployees.filter(
