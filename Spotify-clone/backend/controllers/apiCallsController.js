...
const fetchArtistDetails = async (req, res) => {
    try {
        ...
        res.status(200).json({ name, genres, artistImage, followers: followers.total, popularity });
    } catch (err) {
...
const fetchTrackDetails = async (req, res) => {
    try {
        ...
        res.status(200).json({ spotifyTrackId, title, artist, albumName, albumCover, duration });
    } catch (err) {
